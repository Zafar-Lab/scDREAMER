...
import numpy as np
from utils_new_semi import dense,lrelu,zinb_model,eval_cluster_on_test,load_gene_mtx
import pandas as pd
...
def build_model(self):
    ...
    self.labels = tf.placeholder(dtype = tf.float32, shape=[None, self.N_celltype_exp], name='celltype_input')
    ...
    self.classifier_logit = dense(self.z, self.z_dim, self.N_celltype_exp, name = 'classifier')
    ...
    self.kl_gauss_z_step1 = 0.5 * tf.reduce_sum(- tf.log(tf.math.minimum(tf.math.maximum(self.z_post_v, capL), capU)) \
         + self.z_post_v + tf.square(self.z_post_m) - 1, 1) #step1 - no celltypes_na is infered
    ...
    #recons_lossZ
    self.ELBO_gauss = tf.reduce_mean(recon_loss - self.kl_scale*self.kl_gauss_l - self.kl_scale*self.kl_gauss_z - self.kl_scale*self.kl_gauss_u) \
        - self.kl_scale*tf.reduce_sum(tf.pow(self.z - self.z_, 2)) 

    self.ELBO_gauss_step1 = tf.reduce_mean(recon_loss - self.kl_scale*self.kl_gauss_l - self.kl_scale*self.kl_gauss_z_step1) \
        - self.kl_scale*tf.reduce_sum(tf.pow(self.z - self.z_, 2))  #step1 no cell types_na infered
    ...
    self.classifier_loss = tf.multiply(self.classifier_loss, self.labels_naT)
    self.classifier_loss = 10*tf.reduce_mean(self.classifier_loss)

    #print ("classifier after", self.classifier_loss.eval())
    
    
    self.autoencoder_loss = - self.ELBO_gauss \
    - tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits = self.disb_real_logit, labels = self.batch_input)) \
    - tf.log(tf.math.minimum(tf.math.maximum(tf.reduce_sum(tf.sqrt(tf.abs(self.dis2_real_logit/tf.reduce_sum(self.dis2_real_logit)* self.dis2_fake_logit/tf.reduce_sum(self.dis2_fake_logit)))), capL), capU)) + self.classifier_loss           
                   
    self.autoencoder_loss_step1 = - self.ELBO_gauss_step1 \
    - tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits = self.disb_real_logit, labels = self.batch_input)) \
    - tf.log(tf.math.minimum(tf.math.maximum(tf.reduce_sum(tf.sqrt(tf.abs(self.dis2_real_logit/tf.reduce_sum(self.dis2_real_logit)* self.dis2_fake_logit/tf.reduce_sum(self.dis2_fake_logit)))), capL), capU))     #step1 no cell types_na infered
    # Discriminator D2: minimize distance min max objective function - BD distance between X(generated sample) and X input 
    
    self.dis2_loss = tf.log(tf.math.minimum(tf.math.maximum(tf.reduce_sum(tf.sqrt(tf.abs(self.dis2_real_logit/tf.reduce_sum(self.dis2_real_logit)* self.dis2_fake_logit/tf.reduce_sum(self.dis2_fake_logit)))), capL), capU)) # epsilon added to avoid Nan
...
def train_cluster(self):
    ...
    autoencoder_optimizer_step1 = tf.train.AdamOptimizer(learning_rate = 0.0002*0.5,beta1 = self.beta1).minimize(self.autoencoder_loss_step1)
    # Discriminator D2
    ...
    #for it in range(num_batch_iter):
        if ep==self.step1_epochs: # or (ep>=self.step1_epochs and ep%self.update_na_epoch==0): not updating every epoch
            self.update_na()
        ...
        for it in range(num_batch_iter):
            ...
            if ep<self.step1_epochs:
                batch_x, X_, _, _ = self.next_batch(self.data_train, self.batch_train, None, None, self.train_size,ep) #labels_, labels_n
            
            else : 
                batch_x, X_, labels_, labels_n = self.next_batch(self.data_train, self.batch_train, self.labels_enc, self.labels_na, self.train_size,ep)
                labels_n_ = []
                
                for i in labels_n:
                    if i =="NA":
                        labels_n_ += [[0 for i in range(1)]] 
                    else:
            ...
            if ep<self.step1_epochs:
                _, a_loss_curr = self.sess.run([autoencoder_optimizer_step1, self.autoencoder_loss_step1],
                ...
                _, a_loss_curr, elbo_curr, classifier_loss_curr = self.sess.run([autoencoder_optimizer, self.autoencoder_loss, self.ELBO_gauss,  self.classifier_loss ],
                            feed_dict = {self.x_input: batch_x, self.x_target: batch_x, 
                            self.x_input_: batch_x_, self.batch_input_: X_,
                            self.batch_input: X_, # batch_b
                            self.labels: labels_,
                            self.labels_naT: labels_n_,
                            self.keep_prob: self.keep_param})
                if it%10 == 0 or np.isnan(np.array([a_loss_curr, elbo_curr, classifier_loss_curr])).any() :
                    print('epoch : {}, batch : {},  a_loss_curr : {:.2f}, elbo_curr : {:.2f} classifier_loss_curr : {:.2f}'.format(ep, it,  a_loss_curr, elbo_curr, classifier_loss_curr))
            ...
            _, db_loss_curr = self.sess.run([batchClassifier_optimizer, self.disb_loss],
                        ...
                        self.x_input_: batch_x_, self.batch_input_: X_,
                        self.keep_prob: self.keep_param})
            if np.isnan(np.array([d2_loss_curr, db_loss_curr])).any() :
                print('epoch : {}, batch : {},  d2_loss_curr : {:.2f}, db_loss_curr : {:.2f} '.format(ep, it,  d2_loss_curr, db_loss_curr))
            """
...
def classifier(self, z, reuse = True):
    ...
    out = dense(h, 256, self.N_celltype_exp, name = 'classifier_output')
...
def encoderU(self, z, reuse = False):
    ...
    h = tf.nn.dropout(lrelu(dense(z, self.z_dim + self.N_celltype_exp, self.g_h_dim[0], name='encu_h0_lin'), alpha=self.leak),
                      keep_prob=self.keep_prob)                
...
def decoderU(self, u, reuse = False):
    ...
    h = tf.nn.dropout(lrelu(dense(u , self.u_dim + self.N_celltype_exp, 8, name = 'decu_lin'),alpha = self.leak), keep_prob = self.keep_prob)
...
def update_sudo_lables(adata):
    ...
    for label in adata.obs['celltype_NA'].unique():
        ...
        non_na_percent = 0.5
        no_of_nn = int(sum(is_label)*non_na_percent)
...
def update_na(self):
    ...
    adata=anndata.AnnData(latent_matrix)
    sc.pp.neighbors(adata,use_rep='X',random_state=0) 
    opt_res = find_resolution(adata,self.N_celltype_exp) 
    sc.tl.louvain(adata, resolution = opt_res, key_added='celltype_NA')
    adata = update_sudo_lables(adata)
    self.labels_na = adata.obs['celltype_NA'].tolist()
    
    
    enc = OneHotEncoder(handle_unknown='ignore')
    cell_info = np.setdiff1d(np.unique(self.labels_na),np.array('NA'))
    cell_info = cell_info.reshape(-1,1)
    enc.fit(cell_info)
    self.labels_enc =  enc.transform(np.array(self.labels_na).reshape(-1, 1)).toarray()
    print('created psuedo lables its shape {}, its sum {} '.format(self.labels_enc.shape, np.sum(self.labels_enc)))
...
class scDREAMER(object):
    ...
    def __init__(self, sess, batch, cell_type, name, N_celltype_exp, epoch = 200, lr=0.0007, beta1=0.9, batch_size=128, X_dim=2000, 
                 z_dim=10, dataset_name='Pancreas', step1_epochs = 100,
        ...
        self.N_celltype_exp = N_celltype_exp
        self.step1_epochs = step1_epochs
        # self.update_na_epoch = update_na_epoch
        
        if self.trans == 'sparse':
            self.data_train, self.data_test, self.scale, self.labels_train_gt, self.labels_test_gt, self.batch_train, self.batch_test, self.batch_info, self.labels_enc_gt  = load_gene_mtx(self.dataset_name, transform=False, count=False, actv=self.gen_activation, batch = self.batch, cell_type = self.cell_type, name = self.name)
            self.labels_na = None #_gt ground truth do not use while computing
            
            self.N_batch = self.batch_train.shape[1]
            self.N_celltype_gt = self.labels_enc_gt.shape[1]
            
        else:
            self.data_train, self.data_test, self.labels_train_gt, self.labels_val_gt, self.labels_test_gt  = load_gene_mtx(self.dataset_name, transform=True, batch = self.batch, cell_type = self.cell_type, name = self.name)
            self.scale = 1.0
    ...
    def next_batch(self, data, batch_info, labels=None, labels_na=None, max_size=None, ep=0):

        indx = np.random.randint(max_size - self.batch_size)
        if ep<self.step1_epochs:
            return data[indx:(indx + self.batch_size), :], batch_info[indx:(indx + self.batch_size), :], None, None   
        return data[indx:(indx + self.batch_size), :], batch_info[indx:(indx + self.batch_size), :], labels[indx:(indx + self.batch_size), :], labels_na[indx:(indx + self.batch_size)]
