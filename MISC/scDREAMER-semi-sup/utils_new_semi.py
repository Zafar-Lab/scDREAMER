...
def eval_cluster_on_test(self,ep, name):
    ...
    np.savetxt(name + 'latent_matrix_50c'+str(ep)+'.csv', latent_matrix, delimiter=",")
...
def read_h5ad(data_path, batch, cell_type, name, hvg=2000):
    ...
    labels_na = Ann.obs[cell_type + "_NA"]
    
    return data, labels, batch_info_enc, batch_info, labels_enc, labels_na
...
def load_gene_mtx(dataset_name, name, transform = True, count = True, actv = 'sig', batch = "batch", cell_type = "cell type"):
    ...
    #B = "tech"
    #C = "celltype"
    data, labels, batch_info_enc, batch_info, labels_enc, labels_na = read_h5ad(dataset_name, batch, cell_type, name)
    ...
    if count == False:
        return data, data, scale, labels, labels, batch_info_enc, batch_info_enc, batch_info, labels_enc, labels_na
