...
def build_model(self):
    ...
    self.autoencoder_loss = - self.ELBO_gauss - tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits = self.disb_real_logit, labels = self.batch_input)) 
...
def train_cluster(self):
    ...
    #for it in range(num_batch_iter):
        ...
        #db_loss_curr = 0
        print("Epoch : [%d] ,  a_loss = %.4f,db_loss: %.4f" 
              % (ep, a_loss_curr, db_loss_curr))
        
        
        self._is_train = False # enables false after 1st iterations only...to make training process fast

        if (np.isnan(a_loss_curr) or np.isnan(db_loss_curr)): # np.isnan(d2_loss_curr)
            a_loss_curr = 0
            db_loss_curr = 0
            break
