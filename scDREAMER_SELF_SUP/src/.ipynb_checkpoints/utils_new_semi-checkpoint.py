...
def eval_cluster_on_test(self,ep, name):
    ...
    Ann.obs['label_na'] = self.labels_na

    
    #print (self.labels_ground)
    Ann.obs['ground truth'] = self.labels_ground.values.copy() #.astype(str)
    
    #print (Ann.obs['ground truth'])
    sc.pp.neighbors(Ann, use_rep = 'final_embeddings') #use_rep = 'final_embeddings'
    sc.tl.umap(Ann)
    img = sc.pl.umap(Ann, color = ['ground truth', 'group'], frameon = False) # cells
    print(img)
    
    np.savetxt(name + '_latent_matrix_c' + str(ep) + '.csv', latent_matrix, delimiter=",")
    
    Ann.obs['batch'] = self.batch_info.astype(str)
    img2 = sc.pl.umap(Ann, color = ['batch', 'label_na'], frameon = False)
    print(img2)
    ...
    labels_gt_num  = ord_enc.fit_transform(self.labels_ground)
    
    print('Computing NMI ...')
    NMI = nmi(labels_gt_num.flatten(), y_pred.flatten())
    print('Done !')
...
def read_h5ad(self, data_path, batch, cell_type, name, hvg=2000):
    print('updated hvg')
    ...
    self.enc_cell_1hot = OneHotEncoder(handle_unknown = 'ignore')
    ...
    self.enc_cell_1hot.fit(cell_info.reshape(-1, 1))
    
    labels_enc = self.enc_cell_1hot.transform(cell_info.reshape(-1, 1)).toarray()
    
    labels_na = Ann.obs[self.cell_type_NA].values.copy()    
    labels_ground = Ann.obs[self.plot_cell_type]
    labels_inferred = Ann.obs[cell_type]
    
    return data, labels, batch_info_enc, batch_info, labels_enc, labels_na, labels_ground, labels_inferred


def load_gene_mtx(self, dataset_name, name, transform = True, count = True, actv = 'sig', batch = "batch", cell_type = "cell type"):
    
    print('came in load_gene')

    data, labels, batch_info_enc, batch_info, labels_enc, labels_na, labels_ground, labels_inferred = self.read_h5ad(dataset_name, batch, cell_type, name)
    ...
    if count == False:
        return data, data, scale, labels, labels, batch_info_enc, batch_info_enc, batch_info, labels_enc, labels_na, labels_ground, labels_inferred

    return data, data, labels, labels, labels, labels_ground, labels_inferred
