...
def update_sudo_lables_mean_old(adata, conn_clusters, label_percent):
...
def get_reference_labels(adata, ref_labels, col_name):
    
    old_dict = {}
    new_dict = {}
    
    #col_name = "cell_type_inferred" # celltype_NA
    
    ref_labels = np.array(ref_labels)
    curr_labels =  np.array(adata.obs[col_name].values.copy())
    
    adata.obs[col_name + '_copy'] = adata.obs[col_name].values.copy()
    #adata.obs['celltype_NA'] = adata.obs['celltype_NA'].cat.add_categories(['NA'])
    ...
    for label in np.unique(curr_labels):
        ...
        if label != "NA":    
            ...
            if (highest_match != label):
                ...
                print ("labels", label, " resetting labels to ", highest_match)
                adata.obs[col_name + '_copy'].iloc[new_dict[label]] = highest_match
    
    # Replace with the new value
    #print (adata.obs[col_name].value_counts())
    adata.obs[col_name] = adata.obs[col_name + '_copy'].values.copy()
