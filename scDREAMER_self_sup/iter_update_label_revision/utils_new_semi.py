...
cell_type_key = {'Immune_Human' : 'final_annotation', #'final_annotation',
                 'Lung' : 'cell_type',
                 'Pancreas' : 'celltype',
...
def eval_cluster_on_test(self,ep, name):
    ...
    Ann.obs['label_na'] = self.labels_na.values.copy()
...
def read_h5ad(data_path, batch, cell_type, name, hvg=2000):
    ...
    labels_na = Ann.obs["celltype_NA"]
    labels_ground = Ann.obs[cell_type_key[name]]
...
def load_gene_mtx(dataset_name, name, transform = True, count = True, actv = 'sig', batch = "batch", cell_type = "cell type"):
    ...
    #B = "tech"
    #C = "celltype"
    data, labels, batch_info_enc, batch_info, labels_enc, labels_na, labels_ground, labels_inferred = read_h5ad(dataset_name, batch, cell_type, name)
