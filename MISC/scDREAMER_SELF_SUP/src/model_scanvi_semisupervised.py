...
import numpy as np
from src.utils_new_semi import dense,lrelu,zinb_model,eval_cluster_on_test,load_gene_mtx, read_h5ad
import pandas as pd
...
def build_model(self):
    ...
    """
    #self.classifier_loss = 10*tf.reduce_sum(class_loss_vec)/(self.masked_labels.get_shape()[0] + tf.constant(1))

    #self.classifier_loss = 10*tf.reduce_mean(tf.boolean_mask(class_loss_vec, tf.is_finite(class_loss_vec)))
...
def train_cluster(self):
    ...
    #for it in range(num_batch_iter):
        ...
        if ep % 25 == 0 and ep >= threshold: #50:#and ep != 0: 
            self.update_na()
...
# +
#self.labels_train, self.labels_test, self.labels_enc  self.labels_na

def update_na(self):
    
    print ("Updating clustering", self.dis_conn, " and ", self.conn)
    ...
    opt_res = find_resolution(adata, self.N_celltype) 
    
    if (self.community_algo == "louvain"):
        sc.tl.louvain(adata, resolution = opt_res, key_added = 'celltype_NA')
        
    elif(self.community_algo == "leiden"):
        sc.tl.leiden(adata, resolution = opt_res, key_added = 'celltype_NA')
   
    # cell type inferred update
    adata.obs['cell_type_inferred'] = adata.obs['celltype_NA'].values.copy() 

    
    # Computing connected cluster components...
    A = kneighbors_graph(latent_matrix, 2, mode = 'connectivity', include_self = True)
    A = A.toarray()

    clus_conn, undirected_z_score = compute_undirected_cluster_connectivity(adata.obs['celltype_NA'], A)
    
    a = clus_conn.max(1)
    conn_clusters = a[a >= 0.5].index
    
    print ('conn clusters ', conn_clusters)
    print ('labels', adata.obs['celltype_NA'])
    
    if self.dis_conn == 0.0:
        
        print ("dis_conn", self.dis_conn)
        adata = update_sudo_lables_mean_old(adata, conn_clusters, 1 - self.conn)
        
    else:

        adata = update_sudo_lables(adata, conn_clusters, self.dis_conn) 
        adata = update_sudo_lables_mean(adata, conn_clusters, 1 - self.conn)
    
    # Check label invesion...  sending in old labels...  
    adata = get_reference_labels(adata, self.test_louvain_labels, "cell_type_inferred")
    #adata = get_reference_labels(adata, self.test_louvain_NA, "celltype_NA")
    
    curr_louvain_labels = np.array(adata.obs['cell_type_inferred'].values.copy())
    val = sum(curr_louvain_labels == np.array(self.test_louvain_labels))
    
    print ('Louvain match', val/latent_matrix.shape[0])
    
    # update with the new clustering...
    self.test_louvain_labels = adata.obs['cell_type_inferred'].values.copy()
    self.test_louvain_NA = adata.obs['celltype_NA'].values.copy()
    
    self.labels_na = adata.obs['celltype_NA'].values.copy()
                
    #enc = OneHotEncoder(handle_unknown = 'ignore')
    
    # cell_info = np.setdiff1d(np.unique(self.labels_na),np.array('NA'))
    # cell_info = cell_info.reshape(-1,1)
    print ("unique cell type", adata.obs['celltype_NA'].unique())
    
    #enc.fit(np.array(range(self.N_celltype)).reshape(-1,1))
    
    #fit_transform
    self.labels_enc =  self.enc_cell_1hot.transform(np.array(adata.obs['cell_type_inferred'].values).reshape(-1, 1)).toarray()
    
    self.labels_train =  np.array(adata.obs['cell_type_inferred'])
    self.labels_test = self.labels_train 
...
class scDREAMER(object):
    ...
    def __init__(self, sess, batch, cell_type, cell_type_NA, plot_cell_type, name, epoch = 200, lr=0.0007, beta1=0.9, batch_size=128, X_dim=2000, 
                 z_dim=10, dataset_name='Pancreas', dis_conn = 0.0, conn = 0.5, community_algo = "louvain",
        ...
        self.name = name
        self.batch = batch
        self.dis_conn = dis_conn
        self.conn = conn
        self.community_algo = community_algo
        self.cell_type_NA = cell_type_NA
        self.plot_cell_type = plot_cell_type

        
        if self.trans == 'sparse':
            self.data_train, self.data_test, self.scale, self.labels_train, self.labels_test, self.batch_train, self.batch_test, self.batch_info, self.labels_enc, self.labels_na, self.labels_ground, self.labels_inferred = self.load_gene_mtx(self.dataset_name, transform=False, count=False, actv=self.gen_activation, batch = self.batch, cell_type = self.cell_type, name = self.name)
            
            self.test_louvain_labels = self.labels_inferred # adata.obs['celltype_NA'], labels_inferred
            self.test_louvain_NA = self.labels_na
            self.N_batch = self.batch_train.shape[1]
            self.N_celltype = self.labels_enc.shape[1]
            
        else:
            self.data_train, self.data_test, self.labels_train, self.labels_val,self.labels_test, self.labels_ground  = self.load_gene_mtx(self.dataset_name, transform=True, batch = self.batch, cell_type = self.cell_type, name = self.name)
            self.scale = 1.0
    ...
    def next_batch(self, data, batch_info, labels, labels_na, max_size):
