...
from .utils import (
    ...
    lrelu,
    zinb_model,
    eval_cluster_on_test,
    eval_cluster_on_test_,
    load_gene_mtx,
)
import pandas as pd
...
def build_model(self):
    """
    Build the compleate tensorflow network of the model
    """
    self.x_input = tf.placeholder(
        dtype=tf.float32, shape=[None, self.X_dim], name="Input"
    )
    self.x_input_ = tf.placeholder(
        dtype=tf.float32, shape=[None, self.X_dim], name="Input"
    )
    self.x_target = tf.placeholder(
        dtype=tf.float32, shape=[None, self.X_dim], name="Target"
    )
    self.batch_input = tf.placeholder(
        dtype=tf.float32, shape=[None, self.N_batch], name="batch_input"
    )  # 6, 3
    self.batch_input_ = tf.placeholder(
        dtype=tf.float32, shape=[None, self.N_batch], name="batch_input"
    )

    self.keep_prob = tf.placeholder(dtype=tf.float32, name="keep_prob")
    self.real_distribution = tf.placeholder(
        dtype=tf.float32, shape=[None, self.z_dim], name="Real_distribution"
    )
    ...
    (
        self.encoder_output_,
        self.z_post_m_,
        self.z_post_v_,
        self.l_post_m_,
        self.l_post_v_,
    ) = self.encoder(self.enc_input_, reuse=True)

    self.expression = self.x_input
    self.proj = tf.placeholder(
        dtype=tf.float32, shape=[None, self.X_dim], name="projection"
    )
    ...
    self.x_post_scale = tf.nn.softmax(
        dense(
            self.decoder_output, self.g_h_dim[0], self.n_input, name="dec_x_post_scale"
        )
    )
    ...
    self.x_post_dropout = dense(
        self.decoder_output, self.g_h_dim[0], self.n_input, name="dec_x_post_dropout"
    )
    ...
    self.decoder_output2 = tf.nn.sigmoid(
        dense(self.decoder_output, self.g_h_dim[0], self.X_dim, "dec_output2")
    )

    # Discriminator D1....
    self.dis_real_logit = self.discriminator(
        self.real_distribution, self.z_dim
    )  # random z from Gaussina distribution ...
    self.dis_fake_logit = self.discriminator(
        self.z, self.z_dim, reuse=True
    )  # z distribution coming from encoder...network

    # Discriminator D2
    self.dis2_real_logit = self.discriminator2(self.x_target, self.X_dim)  # True data
    self.dis2_fake_logit = self.discriminator2(
        self.decoder_output2, self.X_dim, reuse=True
    )  # from decoder network
    ...
    # recon_loss = 0
    recon_loss = self.zinb_model(
        self.expression, self.x_post_rate, local_dispersion, self.x_post_dropout
    )
    # recon_loss = tf.reduce_mean(tf.square(tf.subtract( self.decoder_output2, self.x_input)))

    self.kl_gauss_l = 0.5 * tf.reduce_sum(
        -tf.log(tf.math.minimum(tf.math.maximum(self.l_post_v, capL), capU))
        + self.l_post_v / local_l_variance
        + tf.square(self.l_post_m - local_l_mean) / local_l_variance
        + tf.log(tf.math.minimum(tf.math.maximum(local_l_variance, capL), capU))
        - 1,
        1,
    )

    self.kl_gauss_z = 0.5 * tf.reduce_sum(
        -tf.log(tf.math.minimum(tf.math.maximum(self.z_post_v, capL), capU))
        + self.z_post_v
        + tf.square(self.z_post_m)
        - 1,
        1,
    )
    ...
    self.ELBO_gauss = tf.reduce_mean(
        recon_loss - self.kl_scale * self.kl_gauss_l - self.kl_scale * self.kl_gauss_z
    ...
    self.autoencoder_loss = (
        ...
        - tf.reduce_mean(
            tf.nn.softmax_cross_entropy_with_logits(
                logits=self.disb_real_logit, labels=self.batch_input
            )
        )
        - tf.log(
            tf.math.minimum(
                tf.math.maximum(
                    tf.reduce_sum(
                        tf.sqrt(
                            tf.abs(
                                self.dis2_real_logit
                                / tf.reduce_sum(self.dis2_real_logit)
                                * self.dis2_fake_logit
                                / tf.reduce_sum(self.dis2_fake_logit)
                            )
                        )
                    ),
                    capL,
            ...
            tf.math.maximum(
                tf.reduce_sum(
                    tf.sqrt(
                        tf.abs(
                            self.dis_real_logit
                            / tf.reduce_sum(self.dis_real_logit)
                            * self.dis_fake_logit
                            / tf.reduce_sum(self.dis_fake_logit)
                        )
                    )
                ),
                capL,
    ...
    )  # epsilon added to avoid Nan

    # Generator loss - D(z) {D(E(X_real))}  - D1 label will be 1; they are trying to maximize the
    self.generator_loss = -tf.log(
        tf.math.minimum(
            tf.math.maximum(
                tf.reduce_sum(
                    tf.sqrt(
                        tf.abs(self.dis_fake_logit / tf.reduce_sum(self.dis_fake_logit))
                    )
                ),
                capL,
            ),
    ...
    self.gen_vars = [
        var for var in t_vars if "enc_" in var.name or "dec_" in var.name
    ]  # AS:2109

    # Discriminator D2
    self.dis2_vars = [
        var for var in t_vars if "dis2_" in var.name
    ]  # or 'enc_' in var.name or 'dec_' in var.name]
...
def train_cluster(self):
    ...
    autoencoder_optimizer = tf.train.AdamOptimizer(
        learning_rate=0.0002, beta1=self.beta1
    ).minimize(
        self.autoencoder_loss
    )  # self.disb_vars, var_list = self.gen_vars
    ...
    generator_optimizer = tf.train.AdamOptimizer(
        learning_rate=self.lr, beta1=self.beta1
    ).minimize(self.generator_loss, var_list=self.gen_vars)
    # Discriminator D2
    discriminator2_optimizer = tf.train.AdamOptimizer(
        learning_rate=self.lr, beta1=self.beta1
    ).minimize(self.dis2_loss, var_list=self.dis2_vars)

    # Discriminator batch: Classifier....

    discriminatorb_optimizer = tf.train.AdamOptimizer(
        learning_rate=self.lr, beta1=self.beta1
    ...
    for ep in range(self.epoch):
        ...
        for it in range(num_batch_iter):
            ...
            batch_x, X_ = self.next_batch(
                self.data_train, self.batch_train, self.train_size
            )
            ...
            _, a_loss_curr = self.sess.run(
                [autoencoder_optimizer, self.autoencoder_loss],
                feed_dict={
                    self.x_input: batch_x,
                    self.x_target: batch_x,
                    self.x_input_: batch_x_,
                    self.batch_input_: X_,
            ...
            _, d2_loss_curr = self.sess.run(
                [discriminator2_optimizer, self.dis2_loss],
                feed_dict={
                    self.x_input: batch_x,
                    self.x_target: batch_x,
                    self.batch_input: X_,
            ...
            _, db_loss_curr = self.sess.run(
                [discriminatorb_optimizer, self.disb_loss],
                feed_dict={
                    self.x_input: batch_x,
                    self.batch_input: X_,
                    self.real_distribution: batch_z_real_dist,
        ...
        # db_loss_curr = 0
        print(
            "Epoch : [%d] ,  a_loss = %.4f, d_loss: %.4f ,  g_loss: %.4f,  db_loss: %.4f"
        ...
        if (
            np.isnan(a_loss_curr)
            or np.isnan(d_loss_curr)
            or np.isnan(g_loss_curr)
            or np.isnan(db_loss_curr)
        ):  # np.isnan(d2_loss_curr)
            a_loss_curr = 0
            d_loss_curr = 0
            g_loss_curr = 0
            d2_loss_curr = 0
    ...
    with tf.variable_scope("Encoder") as scope:
        ...
        if self.is_bn:
            h = tf.layers.batch_normalization(
                lrelu(
                    dense(
                        x, self.X_dim + self.N_batch, self.g_h_dim[0], name="enc_h0_lin"
                    ),
                    alpha=self.leak,
                ),
            ...
            z_post_m = dense(
                h,
                self.g_h_dim[self.num_layers - 1],
                self.z_dim,
                name="enc_z_post_m" + str(self.num_layers) + "_lin",
            )
            z_post_v = tf.exp(
                dense(
                    h,
                    self.g_h_dim[self.num_layers - 1],
                    self.z_dim,
                    name="enc_z_post_v" + str(self.num_layers) + "_lin",
                )
            )
            ...
            l_post_m = dense(
                h, self.z_dim, 1, name="enc_l_post_m" + str(self.num_layers) + "_lin"
            )
            l_post_v = tf.exp(
                dense(
                    h,
                    self.z_dim,
                    1,
                    name="enc_l_post_v" + str(self.num_layers) + "_lin",
                )
            )
...
def discriminator(self, z, z_dim, reuse=False):
    ...
    :type z: tensor
    :param z_dim: length of each lower dimentional embedding
    :type z_dim: int
            ...
            h = tf.layers.batch_normalization(
                lrelu(
                    dense(
                        z,
                        z_dim,
                        self.d_h_dim[self.num_layers - 1],
                        name="dis_h" + str(self.num_layers - 1) + "_lin",
                    ),
                    alpha=self.leak,
                ),
                training=self._is_train,
                name="dis_bn" + str(self.num_layers - 1),
            )
            ...
            h = tf.layers.batch_normalization(
                lrelu(
                    dense(
                        ...
                        z_dim,
                        self.d_h_dim[self.num_layers - 1],
                        name="dis2_h" + str(self.num_layers - 1) + "_lin",
                    ),
                    alpha=self.leak,
                ),
                training=self._is_train,
                name="dis2_bn" + str(self.num_layers - 1),
            )
            for i in range(self.num_layers - 2, -1, -1):
                h = tf.layers.batch_normalization(
                    lrelu(
    ...
    with tf.variable_scope("Decoder") as scope:
        ...
        if self.is_bn:
            h = tf.layers.batch_normalization(
                ...
                training=self._is_train,
                name="dec_bn" + str(self.num_layers - 1),
            )
            for i in range(self.num_layers - 2, -1, -1):
                h = tf.layers.batch_normalization(
                    lrelu(
            ...
            h = tf.layers.batch_normalization(
                lrelu(
                    dense(
                        ...
                        z_dim,
                        self.d_h_dim[self.num_layers - 1],
                        name="disb_h" + str(self.num_layers - 1) + "_lin",
                    ),
                    alpha=self.leak,
                ),
                training=self._is_train,
                name="disb_bn" + str(self.num_layers - 1),
            )
...
class scDREAMER(object):
    ...
    def next_batch(self, data, batch_info, max_size):
        ...
        return (
            data[indx : (indx + self.batch_size), :],
            batch_info[indx : (indx + self.batch_size), :],
        )
