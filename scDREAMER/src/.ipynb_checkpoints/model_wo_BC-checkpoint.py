...
def train_cluster(self):
    ...
    #for it in range(num_batch_iter):
        ...
        #db_loss_curr = 0
        print("Epoch : [%d] ,  a_loss = %.4f, d_loss: %.4f ,  g_loss: %.4f" 
              % (ep, a_loss_curr, d_loss_curr, g_loss_curr))
        
        
        self._is_train = False # enables false after 1st iterations only...to make training process fast

        if (np.isnan(a_loss_curr) or np.isnan(d_loss_curr) or np.isnan(g_loss_curr)): # np.isnan(d2_loss_curr)
          a_loss_curr = 0
          d_loss_curr = 0
          g_loss_curr = 0
          d2_loss_curr = 0
