...
def build_model(self):
    ...
    self.ELBO_gauss_step1 = tf.reduce_mean(recon_loss - self.kl_scale*self.kl_gauss_l - self.kl_scale*self.kl_gauss_z_step1 - self.kl_scale*self.kl_gauss_u) \
    ...
    self.classifier_loss = tf.reduce_sum(self.classifier_loss)/tf.cast(tf.count_nonzero(self.classifier_loss),tf.float32) # 10*
    ...
    self.autoencoder_loss = - self.ELBO_gauss \
        - tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits = self.disb_real_logit, labels = self.batch_input))\
            - tf.log(tf.math.minimum(tf.math.maximum(tf.reduce_sum(tf.sqrt(tf.abs(self.dis2_real_logit/tf.reduce_sum(self.dis2_real_logit)* self.dis2_fake_logit/tf.reduce_sum(self.dis2_fake_logit)))), capL), capU))\
                 + self.classifier_loss           
                   
    self.autoencoder_loss_step1 = - self.ELBO_gauss_step1 \
        - tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits = self.disb_real_logit, labels = self.batch_input))\
            - tf.log(tf.math.minimum(tf.math.maximum(tf.reduce_sum(tf.sqrt(tf.abs(self.dis2_real_logit/tf.reduce_sum(self.dis2_real_logit)* self.dis2_fake_logit/tf.reduce_sum(self.dis2_fake_logit)))), capL), capU))\
                  #step1 no cell types_na infered
...
def train_cluster(self):
    ...
    #for it in range(num_batch_iter):
        if ep==self.step1_epochs or (ep>=self.step1_epochs and ep%self.update_na_epoch==0):
            self.update_na()
...
def update_na(self):
    ...
    opt_res = find_resolution(adata,self.N_celltype) #,no_of_cell_types) need to get optimal number of clusters
...
class scDREAMER(object):
    ...
    def __init__(self, sess, batch, cell_type, name, epoch = 200, lr=0.0007, beta1=0.9, batch_size=128, X_dim=2000, 
        ...
        self.name = name
        self.batch = batch
        self.step1_epochs = 100
        self.update_na_epoch = 10
    ...
    def next_batch(self, data, batch_info, labels, labels_na, max_size, ep=0):

        indx = np.random.randint(max_size - self.batch_size)
        if ep<self.step1_epochs:
            return data[indx:(indx + self.batch_size), :], batch_info[indx:(indx + self.batch_size), :], 0,0    
