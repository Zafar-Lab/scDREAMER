...
def update_na(self):
    ...
    # Updating labels for only connected cluster components...
    adata = update_sudo_lables(adata, conn_clusters, 0.1)
    
    conn_clusters = []
    adata = update_sudo_lables_mean(adata, conn_clusters, 0.5)
    
    # Check label invesiob...
    adata = get_reference_labels(adata, self.test_louvain_labels)
    
    #curr_louvain_labels = np.array(adata.obs['celltype_NA'].values.copy())
    ...
    self.test_louvain_labels = adata.obs['celltype_NA'].values.copy()
    
    self.labels_na = adata.obs['celltype_NA'].tolist()
    
    #not making na cells as zero hotted as it is causing issue in the classifier loss
    
    adata.obs['cell_type_inferred'] = adata.obs['celltype_NA'].values.copy()       
    adata.obs['cell_type_inferred'].loc[adata.obs['cell_type_inferred'] == 'NA'] = '0'
    ...
    enc.fit(list(range(self.N_celltype)))
    
    #adata.obs['cell_type_inferred'] = adata.obs['cell_type_inferred'].remove_unused_categories()
    print (np.array(adata.obs['cell_type_inferred'].values))
    #print (adata.obs['cell_type_inferred'].values.reshape(-1, 1))
    
    #fit_transform
    self.labels_enc =  enc.transform(np.array(adata.obs['cell_type_inferred'].values).reshape(-1, 1)).toarray()
...
class scDREAMER(object):
    ...
    def __init__(self, sess, batch, cell_type, name, epoch = 200, lr=0.0007, beta1=0.9, batch_size=128, X_dim=2000, 
        ...
        if self.trans == 'sparse':
            self.data_train, self.data_test, self.scale, self.labels_train, self.labels_test, self.batch_train, self.batch_test, self.batch_info, self.labels_enc, self.labels_na, self.labels_ground, self.labels_inferred = load_gene_mtx(self.dataset_name, transform=False, count=False, actv=self.gen_activation, batch = self.batch, cell_type = self.cell_type, name = self.name)
            
            self.test_louvain_labels = self.labels_na # adata.obs['celltype_NA'], labels_inferred
            self.N_batch = self.batch_train.shape[1]
            self.N_celltype = self.labels_enc.shape[1]
            
        else:
            self.data_train, self.data_test, self.labels_train, self.labels_val, self.labels_test, self.labels_ground  = load_gene_mtx(self.dataset_name, transform=True, batch = self.batch, cell_type = self.cell_type, name = self.name)
            self.scale = 1.0
