...
def train_cluster(self):
    ...
    autoencoder_optimizer = tf.train.AdamOptimizer(learning_rate = 0.0002*2,beta1 = self.beta1).minimize(self.autoencoder_loss) #self.disb_vars, var_list = self.gen_vars

    # Discriminator D2
    
    discriminator2_optimizer = tf.train.AdamOptimizer(learning_rate=self.lr*2,
                                                      beta1=self.beta1).minimize(self.dis2_loss,
                                                                                  var_list=self.dis2_vars)
    
    # Discriminator batch: Classifier....
    
    batchClassifier_optimizer = tf.train.AdamOptimizer(learning_rate = self.lr*2,
